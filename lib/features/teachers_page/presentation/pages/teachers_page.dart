  ...
  void _setDate(BuildContext context, DateTime newDate) {
    setState(() {
      ...
      context.read<TeachersPageBloc>().add(
        TeacherLessonsRequestedForDateEvent(name: name, date: newDate),
      );
    });
  }
        ...
        child: BlocBuilder<TeachersPageBloc, TeachersPageBlocState>(
          builder: (context, state) {
            return switch (state) {
              ...
              TeachersPageTeachersErrorState(error: final error) =>
                ErrorMessageBlock(
                  errorMessage: error,
                  circumstances: "Загрузке преподавателей",
                  onRetry: () => context.read<TeachersPageBloc>().add(
                    TeachersPageInitEvent(),
                  ),
                ),
                    ...
                    if (state.lessons != null)
                      Row(
                        ...
                        children: [
                          IconButton(
                            onPressed: () =>
                                _setDate(context, date.add(Duration(days: -7))),
                            icon: const Icon(Icons.arrow_circle_left_outlined),
                            color: context.colors.primary,
                          ),
                          IconButton(
                            onPressed: () =>
                                _setDate(context, date.add(Duration(days: -1))),
